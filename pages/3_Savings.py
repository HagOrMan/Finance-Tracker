...
st.set_page_config(
    page_title="Savings | " + config.APP_TITLE, page_icon=config.APP_ICON, layout="wide"
)
...
num_receipts_with_discount = (
    (df["discount"] > 0) | (df["discount_percentage"] > 0)
).sum()
...
k3.metric(
    "Savings rate",
    (
        f"{(total_savings / (total_spend + total_savings) * 100):.1f}%"
        if (total_spend + total_savings) > 0
        else "—"
    ),
)
...
fig = charts.savings_over_time(
    df, savings_col="total_savings", date_range_days=date_range_days
)
