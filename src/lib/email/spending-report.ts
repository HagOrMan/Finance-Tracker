...
// `font-weight` is explicit because these hints now nest inside bold labels and
// would otherwise inherit the weight and stop reading as an aside.
const faintText = `${base}font-size:12px;line-height:1.45;color:${C.faint};font-weight:400;`;
...
function headlineSection(report: SpendingReport): string {
  ...
  // A filled cell, not a border. See ARCHITECTURE.md §6 — the borders that used
  // to separate these rows sat after `font-family` in the same style attribute
  // and were discarded along with everything else, so this template has never
  // actually shown a rule anywhere.
  const divider = `<tr><td colspan="2" height="1" style="height:1px;line-height:1px;font-size:1px;background-color:${C.rule};">&nbsp;</td></tr>`;

  const stat = (name: string, value: string, hint: string) =>
    `<tr>
       <td style="${mutedText}padding:10px 12px 10px 0;font-weight:600;">${escapeHtml(name)}<div style="${faintText}margin-top:2px;">${escapeHtml(hint)}</div></td>
       <td align="right" style="${bodyText}padding:10px 0;font-weight:700;white-space:nowrap;vertical-align:top;">${value}</td>
     </tr>`;
...
function categorySection(
  ...
  const rows = categories
    .map((row, i) => {
      ...
      return `${i === 0 ? "" : divider}
        <tr>
          <td style="${bodyText}padding:12px 12px 0 0;font-weight:600;">${escapeHtml(row.category)}</td>
          <td align="right" style="${bodyText}padding:12px 0 0;font-weight:700;white-space:nowrap;">${formatCurrency(row.spent)}</td>
        </tr>
        <tr><td colspan="2" style="padding:6px 0 0;background-color:${C.cardBg};">${barHtml(barPercent(row.spent, max), color)}</td></tr>
        <tr><td colspan="2" style="${faintText}padding:5px 0 12px;">${share}% of habitual · <span style="color:${changeColor(row.changeVsBaseline)};background-color:${C.cardBg};">${escapeHtml(changeText)}</span></td></tr>`;
    })
    .join("");

  const hidden = hiddenCategories
    ? `${divider}<tr><td colspan="2" style="${mutedText}padding:12px 0 0;">${hiddenCategories.count} other categor${hiddenCategories.count === 1 ? "y" : "ies"} — ${formatCurrency(hiddenCategories.spent)}</td></tr>`
    : "";
  ...
  const rows = excluded.categories
    .map(
      (row, i) =>
        `${i === 0 ? "" : divider}
         <tr>
           <td style="${bodyText}padding:10px 12px 10px 0;font-weight:600;">${escapeHtml(row.category)}</td>
           <td align="right" style="${bodyText}padding:10px 0;white-space:nowrap;">${formatCurrency(row.spent)}</td>
         </tr>`,
    )
    .join("");
  ...
  return sectionHtml(
    ...
    `<div style="${mutedText}margin:-4px 0 10px;">Real spending, held out of the figures above because it isn&rsquo;t habitual — it would drown out everything the comparison exists to show.</div>
     <table role="presentation" cellpadding="0" cellspacing="0" border="0" width="100%" style="border-collapse:collapse;">
       ...
       ${divider}
       <tr>
         <td style="${bodyText}padding:10px 12px 0 0;font-weight:700;">All-in total</td>
         <td align="right" style="${bodyText}padding:10px 0 0;font-weight:700;white-space:nowrap;">${formatCurrency(allInSpent)}</td>
       </tr>
     </table>`,
  );
}
