...
with tc1:
    sel_cats = st.multiselect("Filter by category", tbl_cats, key="tbl_ov_cat")
with tc2:
    sel_stores = st.multiselect("Filter by store", tbl_stores, key="tbl_ov_store")
with tc3:
    note_search = st.text_input(
        "Search notes", key="tbl_ov_note", placeholder="type to search…"
    )
